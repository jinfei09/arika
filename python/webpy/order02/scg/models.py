...
class AccountManager(models.Manager):
    ...
    def need_service(self, scg_character):
...
class OrderManager(models.Manager):
    def create_order(self, account, food_id, date, scg_character, remark):
        try:
            food = Food.objects.get(pk=food_id)
            order = Order(username=account, restaurant=food.restaurant, food=food,
                          price=food.price, date=date, character=scg_character,
                          remark=remark)
            order.save()
        except:
            return False
    ...
    def get_process_order(self, date, scg_character, status='b'):
        query = Order.objects.filter(date=date, character=scg_character, 
...
def pay_scg(interval, character, subsidy):
...
class PayRecordManager(models.Manager):
    def finish_pay(self):
        settlement = True
        pay_scg(30, 'l', 8) # check_interval, scg character,scg subsidy
        pay_scg(30, 'd', 15)
        return settlement
